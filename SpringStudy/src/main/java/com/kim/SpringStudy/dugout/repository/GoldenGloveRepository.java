...
public interface GoldenGloveRepository extends JpaRepository<KBOplayerInfo, Long> {
    ...
    @Query(value = """
                SELECT
                    ...
                    p.position_sub,
                    CAST((
                   SUM(s.obp) * 20 + SUM(s.slg) * 20 + SUM(s.hr) * 3 + SUM(s.rbi) * 1.2 + SUM(s.avg) * 40 +
                   SUM(s.risp) * 15 + SUM(s.mh) * 0.5 -
    ...
    @Query(value = """
                SELECT
                    ...
                    p.position,
                    CAST((
                           SUM(s.w) * 3 + SUM(s.sv) * 1.5 + SUM(s.hld) * 1.5 + SUM(s.so) * 0.3 + SUM(s.qs) * 2 -
                           SUM(s.era) * 20 - SUM(s.whip) * 10 + (SUM(s.ip) / COUNT(s.id)) * 0.5 -
